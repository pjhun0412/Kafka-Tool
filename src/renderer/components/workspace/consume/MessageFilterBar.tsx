  ...
  return (
    <div className="filter-bar">
      ...
      <div className="filter-input-wrap">
        <input value={props.filterDraft} onChange={(event) => props.onFilterDraft(event.target.value)} placeholder={t(language, "placeholder.filterMessages")} />
      </div>
      <div className="filter-help">
        <IconButton variant="subtle" aria-label={t(language, "title.filterHelp")}>
          <HelpCircle size={15} />
        </IconButton>
        <div className="filter-help-popover" role="tooltip">
          <strong>{t(language, "label.filterSyntax")}</strong>
          <div className="filter-help-content">
            ...
            <span className="filter-help-note">{t(language, "label.fields")}: key, value, headers, partition, offset, timestamp, topic, decoded</span>
          </div>
        </div>
      </div>
      <Button
        ...
        title={props.filterMode === "hide" ? t(language, "title.hideUnmatchedRows") : t(language, "title.highlightMatchedRows")}
      >
        {props.filterMode === "hide" ? <EyeOff size={14} /> : <Sparkles size={14} />}
        {props.filterMode === "hide" ? t(language, "label.hide") : t(language, "label.highlight")}
      </Button>
      <Button variant="ghost" size="sm" onClick={props.onClear}>{t(language, "label.clear")}</Button>
    </div>
  );
}
