...
import { BrokersPanel, ConsumePanel, ConsumerGroupsPanel, PaneToastView, ProducePanel, ServerTopicsPanel, SplitWorkspacePane, TopicListItem, TopicPanel } from "./components/workspace/WorkspacePanels";
...
function App() {
  ...
  const [topicGridSortingByServer, setTopicGridSortingByServer] = useState<Record<string, SortingState>>({});
  ...
  const [topicDetailCacheByServer, setTopicDetailCacheByServer] = useState<Record<string, Record<string, TopicDetail>>>({});
  ...
  function isSplitWorkspaceActiveFor(serverId: string) {
    return Boolean(visibleSplitPane && visibleSplitPane.serverId === serverId);
  }

  function getWorkspaceTargetForServer(serverId = selectedServerId, topic?: string): WorkspaceActionTarget {
    if (activeWorkspacePane === "split" && isSplitWorkspaceActiveFor(serverId)) {
      return {
        pane: "split",
        serverId,
        topic: topic ?? visibleSplitPane?.topic
      };
    }
    return {
      pane: "primary",
      serverId,
      topic: topic ?? selectedTopicByServer[serverId] ?? ""
    };
  }

  function addTopicTab(tabs: string[], topic: string) {
    return tabs.includes(topic) ? tabs : [...tabs, topic];
  }

  function removeTopicTab(tabs: string[], topic: string) {
    return tabs.filter((item) => item !== topic);
  }

  function getNextTopicAfterTabClose(currentTopic: string, closedTopic: string, nextTabs: string[]) {
  ...
  function updateTopicGridSortingForServer(serverId: string, updater: SortingState | ((current: SortingState) => SortingState)) {
  ...
  function activateSplitSelectedTopicView() {
    ...
    if (nextView === "info" && !pane.detail) {
      void loadSplitTopicDetailSilent(pane.serverId, pane.topic);
    }
  }
  ...
  async function selectTopicInWorkspace(target: WorkspaceActionTarget, topic: string) {
    ...
    if (nextView === "info" && kafkaApi) {
      ...
      const detail = await runWorkspaceTask({ pane: "primary", serverId: target.serverId, topic }, workspaceMessages.topicDetailLoading, () => kafkaApi.getTopicDetail(target.serverId, topic));
      setTopicDetailForServer(target.serverId, detail);
    }
  }

  async function openTopicInWorkspace(target: WorkspaceActionTarget, topic: string, nextView: TopicWorkView = getTopicViewFor(target.serverId, topic)) {
    if (!topic) return;
    if (target.pane === "split") {
  ...
  async function runWorkspaceTask<T>(target: WorkspaceActionTarget, label: string, task: () => Promise<T>) {
    if (isSplitWorkspaceActiveFor(target.serverId)) {
      return runPaneTask(target.pane, label, task);
    }
    return runTask(label, task);
  }
  ...
  async function loadSplitTopicDetail(serverId: string, topic: string) {
    ...
    const detail = await runPaneTask("split", workspaceMessages.topicDetailLoading, () => kafkaApi.getTopicDetail(serverId, topic));
    setTopicDetailCacheByServer((current) => ({
      ...current,
      [serverId]: {
        ...(current[serverId] ?? {}),
  ...
  async function openPrimaryTopicTab(topic: string) {
    setOpenedTopicTabs((current) => addTopicTab(current, topic));
  ...
  async function closeSplitTopicTab(topic: string) {
    ...
    const nextTabs = removeTopicTab(pane.topicTabs, topic);
    if (nextTabs.length === 0) {
      setSplitPane(null);
      setActiveWorkspacePane("primary");
      return;
    }
    const nextTopic = getNextTopicAfterTabClose(pane.topic, topic, nextTabs);
  ...
  async function promoteSplitPaneToPrimary() {
    ...
    setOpenedTopicTabsByServer((current) => {
      ...
      return { ...current, [pane.serverId]: mergedTabs };
    });
    ...
    setConsumeStatesByServer((current) => ({
      ...current,
      [pane.serverId]: {
        ...(current[pane.serverId] ?? {}),
        ...(splitConsumeStatesByServer[pane.serverId] ?? {})
      }
    }));
    setSplitConsumeStatesByServer((current) => {
      const next = { ...current };
      delete next[pane.serverId];
      return next;
    });
  ...
  async function refreshTopicsForServer(serverId: string, target?: WorkspaceActionTarget) {
    if (!kafkaApi) return;
    setTopicQueryByServer((current) => ({ ...current, [serverId]: "" }));
    const items = target
      ? await runWorkspaceTask(target, "토픽 조회 중", () => kafkaApi.listTopics(serverId))
      : await runTask("토픽 조회 중", () => kafkaApi.listTopics(serverId));
    ...
    if (nextTopic) {
      const detail = target
        ? await runWorkspaceTask({ ...target, topic: nextTopic }, workspaceMessages.topicDetailLoading, () => kafkaApi.getTopicDetail(serverId, nextTopic))
        : await runTask(workspaceMessages.topicDetailLoading, () => kafkaApi.getTopicDetail(serverId, nextTopic));
  ...
  async function loadTopicDetail(topic: string) {
    ...
    setSelectedTopic(topic);
    const detail = await runTask(workspaceMessages.topicDetailLoading, () => kafkaApi.getTopicDetail(selectedServerId, topic));
    setTopicDetailForServer(selectedServerId, detail);
  }
  ...
  async function loadConsumerGroupLag(groupId: string) {
    ...
    const target = visibleSplitPane ? { pane: "primary", serverId: selectedServerId } satisfies WorkspaceActionTarget : undefined;
    const detail = target
      ? await runWorkspaceTask(target, workspaceMessages.consumerGroupLagLoading, () => kafkaApi.getConsumerGroupLag(selectedServerId, groupId))
      : await runTask(workspaceMessages.consumerGroupLagLoading, () => kafkaApi.getConsumerGroupLag(selectedServerId, groupId));
  ...
  async function loadConsumerGroupLagFor(serverId: string, groupId: string, target?: WorkspaceActionTarget) {
    ...
    const detail = target
      ? await runWorkspaceTask(target, workspaceMessages.consumerGroupLagLoading, () => kafkaApi.getConsumerGroupLag(serverId, groupId))
      : await runTask(workspaceMessages.consumerGroupLagLoading, () => kafkaApi.getConsumerGroupLag(serverId, groupId));
    setGroupLagByServer((current) => ({
      ...current,
      [serverId]: {
        ...(current[serverId] ?? {}),
  ...
  async function refreshCurrentView(pane?: WorkspacePaneId) {
    ...
    if (view === "info") {
      if (selectedTopic) {
        if (pane) {
          await runPaneTask(pane, workspaceMessages.topicDetailLoading, () => loadTopicDetailSilent(selectedTopic, { force: true }));
          return;
        }
        await loadTopicDetail(selectedTopic);
      }
    ...
    if (view === "consume") {
      ...
      if (pane) {
        showPaneToast(pane, workspaceMessages.consumeReset);
      }
      return;
    }
    ...
    if (pane) {
      showPaneToast(pane, workspaceMessages.produceReset);
    }
  }
  ...
  async function refreshSplitPaneView(pane: SplitPaneState, state: TopicConsumeState) {
    ...
    if (pane.view === "info") {
      if (pane.topic) {
        await runPaneTask("split", workspaceMessages.topicDetailLoading, () => loadSplitTopicDetailSilent(pane.serverId, pane.topic, { force: true }));
      }
      return;
    }
    if (pane.view === "consume") {
      ...
      setStatus(workspaceMessages.consumeReset);
      showPaneToast("split", workspaceMessages.consumeReset);
      return;
    }
    if (pane.view === "produce") {
      updateProduceDraftFor(pane.serverId, pane.topic, emptyProduceDraft);
      setStatus(workspaceMessages.produceReset);
      showPaneToast("split", workspaceMessages.produceReset);
    }
  }
  ...
  async function refreshActiveWorkspaceView() {
    ...
    await refreshCurrentView(visibleSplitPane ? "primary" : undefined);
  }
    ...
    >
            ...
            <div className={topicFilter === "favorites" ? "favorite-topic-section fill-available" : "favorite-topic-section"}>
              ...
              {favoriteTopics.map((topic) => (
                <TopicListItem
                  ...
                  onSelect={() => {
                    void selectTopicInWorkspace(getWorkspaceTargetForServer(selectedServerId, topic.name), topic.name);
                  }}
      ...
      >
        ...
        <nav className="tabs server-work-tabs" onMouseDown={(event) => event.stopPropagation()}>
          <button
            className={activeWorkspaceView === "brokers" ? "active" : ""}
            onClick={() => showServerViewInActivePane("brokers")}
            disabled={!isSelectedServerConnected}
          >
            <HardDrive size={16} /> Brokers
          </button>
          <button
            className={activeWorkspaceView === "topics" ? "active" : ""}
            onClick={() => showServerViewInActivePane("topics")}
            disabled={!isSelectedServerConnected}
          >
            <ListTree size={16} /> Topics
          </button>
          <button
            className={activeWorkspaceView === "consumers" ? "active" : ""}
            onClick={() => showServerViewInActivePane("consumers")}
            disabled={!isSelectedServerConnected}
          >
            <Users size={16} /> Consumers
          </button>
          <button className="ghost" onClick={() => void refreshActiveWorkspaceView()} disabled={!isSelectedServerConnected || loading}><RefreshCw size={16} /> 새로고침</button>
        </nav>
        ...
        <div className="content-grid">
          ...
          {view === "topics" && (
            <ServerTopicsPanel
              ...
              sorting={topicGridSortingByServer[selectedServerId] ?? []}
              onSortingChange={(updater) => updateTopicGridSortingForServer(selectedServerId, updater)}
          ...
          {view === "consumers" && (
            <ConsumerGroupsPanel
              ...
              onRefresh={() => void refreshGroupsForServer(selectedServerId, visibleSplitPane ? { pane: "primary", serverId: selectedServerId, topic: selectedTopic } : undefined)}
              onRefreshDetail={() => {
        ...
        {visibleSplitPane && splitServer && (
          <SplitWorkspacePane
            ...
            onView={(nextView) => {
              ...
              if (nextView === "brokers" && !brokersByServer[visibleSplitPane.serverId]) void refreshBrokersForServer(visibleSplitPane.serverId, { pane: "split", serverId: visibleSplitPane.serverId, topic: visibleSplitPane.topic });
              if (nextView === "topics" && !topicsByServer[visibleSplitPane.serverId]) void refreshTopicsForServer(visibleSplitPane.serverId, { pane: "split", serverId: visibleSplitPane.serverId, topic: visibleSplitPane.topic });
              if (nextView === "consumers" && !groupsByServer[visibleSplitPane.serverId]) void refreshGroupsForServer(visibleSplitPane.serverId, { pane: "split", serverId: visibleSplitPane.serverId, topic: visibleSplitPane.topic });
            }}
            onTopic={(topic) => {
            ...
            onToggleTopicSelected={toggleTopicRow}
            onToggleAllTopicsSelected={toggleAllTopicRows}
            onCopySelectedTopics={() => void copySelectedTopicNames()}
            onPurgeSelectedTopics={() => requestTopicAction("purge")}
            onDeleteSelectedTopics={() => requestTopicAction("delete")}
            onToggleTopicFavorite={toggleFavoriteTopic}
            onSelectGroup={(groupId) => void loadConsumerGroupLagFor(visibleSplitPane.serverId, groupId, { pane: "split", serverId: visibleSplitPane.serverId, topic: visibleSplitPane.topic })}
            onBackGroup={() => setSelectedGroupByServer((current) => ({ ...current, [visibleSplitPane.serverId]: "" }))}
            onRefreshGroups={() => void refreshGroupsForServer(visibleSplitPane.serverId, { pane: "split", serverId: visibleSplitPane.serverId, topic: visibleSplitPane.topic })}
            onRefreshGroupDetail={() => {
              if (splitSelectedGroupId) void loadConsumerGroupLagFor(visibleSplitPane.serverId, splitSelectedGroupId, { pane: "split", serverId: visibleSplitPane.serverId, topic: visibleSplitPane.topic });
            }}
