...
Message Viewer 툴바의 Message Replay로 선택한 Consume 메시지를 원하는 대상 서버와 토픽으로 다시 전송할 수 있습니다. Replay 팝업은 원본 Cluster/Topic/Partition/Offset을 보여주고, 대상 서버/토픽 선택과 Key, Headers, Value 포함 여부를 지원합니다. 전송 전 Payload를 직접 수정할 수 있으며 `${uuid}`, `${seq:1..100}`, `${date:yyyy-MM-dd HH:mm:ss}` 같은 Produce Dynamic Field도 Replay 전송 직전에 치환됩니다.
